...
static int rbus_recv(const char *topic, pid_t pid, int *rbus_send_status, rbusGtestMsg_t test)
{
  ...
  ret |= rbusMessage_AddListener(handle, topic, &rbusRecvHandler, (void *)(&userData), &listenerId);
  EXPECT_EQ(ret,RBUS_ERROR_SUCCESS);
    ...
    case RBUS_GTEST_MSG9:
      {
          ...
          EXPECT_EQ(rbusMessage_AddListener(rbus, "A.B.C", &rbusRecvHandler, NULL, &listenerId),0);
          EXPECT_NE(rbusMessage_AddListener(rbus, "A.B.C", &rbusRecvHandler, NULL, &listenerId),0);
          EXPECT_EQ(rbusMessage_RemoveListener(rbus, "A.B.C", listenerId),0);
