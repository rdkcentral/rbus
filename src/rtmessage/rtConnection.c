...
rtConnection_CreateInternal(rtConnection* con, char const* application_name, char const* router_config, int max_retries)
{
  ...
  if (err == RT_OK)
  {
    rtConnection_AddListener(c, c->inbox_name, onDefaultMessage, c, &listenerId, 0);
...
rtError
rtConnection_AddListener(rtConnection con, char const* expression, rtMessageCallback callback, void* closure, uint32_t** listenerId, uint32_t subscriptionId)
{
  int i;
  ...
  **listenerId = con->listeners[i].subscription_id;
