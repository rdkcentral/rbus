            ...
            *numValues = 0;
            ...
            else
            {
                for(i = 0; i < numDestinations; i++)
                {
                    ...
                    /* Invoke the method */
                    err = rbuscore_invokeRemoteMethod(destinations[i], METHOD_GETPARAMETERVALUES, request, rbusConfig_ReadGetTimeout(), &response);
        ...
        rbusEventSubscriptionInternal_t* subInternal)
{
    ...
    coreerr = rbuscore_unsubscribeFromEvent(NULL, subscription->eventName, payload);
    ...
    bool                            publishOnSubscribe)
{
    ...
    for(;;)
    {
        ...
        coreerr = rbuscore_subscribeToEventTimeout(NULL, sub->eventName, _event_callback_handler, payload, sub, &providerError, destNotFoundTimeout, publishOnSubscribe, &response);
    ...
    char const*         eventName)
{
    ...
    if(subInternal)
    {
        ...
        rbusCoreError_t coreerr = rbuscore_unsubscribeFromEvent(NULL, eventName, payload);
...
rbusError_t rbus_createSession(rbusHandle_t handle, uint32_t *pSessionId)
{
    ...
    {
        *pSessionId = 0;
        if((err = rbuscore_invokeRemoteMethod(RBUS_SMGR_DESTINATION_NAME, RBUS_SMGR_METHOD_REQUEST_SESSION_ID, NULL, rbusConfig_ReadSetTimeout(), &response)) == RBUSCORE_SUCCESS)
        {
...
rbusError_t rbus_getCurrentSession(rbusHandle_t handle, uint32_t *pSessionId)
{
    ...
    {
        *pSessionId = 0;
        if((err = rbuscore_invokeRemoteMethod(RBUS_SMGR_DESTINATION_NAME, RBUS_SMGR_METHOD_GET_CURRENT_SESSION_ID, NULL, rbusConfig_ReadGetTimeout(), &response)) == RBUSCORE_SUCCESS)
        {
            rbusMessage_GetInt32(response, /*MESSAGE_FIELD_RESULT,*/ (int*) &err);
            if(RBUSCORE_SUCCESS != err)
            {
...
rbusError_t rbus_closeSession(rbusHandle_t handle, uint32_t sessionId)
{
    ...
    if (handle)
    {
        ...
        if((err = rbuscore_invokeRemoteMethod(RBUS_SMGR_DESTINATION_NAME, RBUS_SMGR_METHOD_END_SESSION, inputSession, rbusConfig_ReadSetTimeout(), &response)) == RBUSCORE_SUCCESS)
        {
            rbusMessage_GetInt32(response, /*MESSAGE_FIELD_RESULT,*/ (int*) &err);
            if(RBUSCORE_SUCCESS != err)
            {
