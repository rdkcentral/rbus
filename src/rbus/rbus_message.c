    ...
    uint32_t *listenerId)
{
    ...
    rtError e = rtConnection_AddListener(myConn, rawDataTopic, &rtMessage_CallbackHandler, ctx, &listenerId, subscriptionId);
    if (e != RT_OK)
    {
    ...
    uint32_t *listenerId)
{
    ...
    rtError e = rtConnection_AddListener(con, expression, &rtMessage_CallbackHandler, ctx, &listenerId, 0);
    if (e != RT_OK)
    {
        RBUSLOG_WARN("rtConnection_AddListener:%s", rtStrError(e));
        return RBUS_ERROR_BUS_ERROR;
    ...
    uint32_t listenerId)
{
    ...
    rtError e = rtConnection_RemoveListenerWithId(myConn, listenerId);
    if (e != RT_OK)
    {
    ...
    uint32_t listenerId)
{
    ...
    rtError e = rtConnection_RemoveListenerWithId(con, listenerId);
    if (e != RT_OK)
    {
        RBUSLOG_WARN("rtConnection_RemoveListener:%s", rtStrError(e));
        return RBUS_ERROR_BUS_ERROR;
